...
// 		    var_dump($method,$params);
	...
	function eth_call($message, $block)
	{
		...
		else
		{
		    ...
		    array_push($message, $block);
			return $this->ether_request(__FUNCTION__, $message);
		}
	}
	...
	{
	    return $this->ether_request(__FUNCTION__,array($account,$password));
	}
	function personal_lockAccount($account)
	{
	    return $this->ether_request(__FUNCTION__,array($account));
	}
}
